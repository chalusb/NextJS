      ...
      <div className="flex">
          ...
          >
            <div className="flex flex-col">
              {leftMenu.items?.map((item) => (
                <div
                  key={item.name}
                  className="block px-4 py-3 text-[#0276b3] hover:bg-gray-100 cursor-pointer"
                >
                  {item.name}
                </div>
              ))}
        ...
        <div className="hidden lg:flex flex-1 bg-[#0276b3] text-white justify-end pr-6">
          <div className="flex items-center space-x-6">
            {rightMenu.map((item) => (
              <div key={item.name} className="text-white hover:text-white/90 py-2 text-sm cursor-pointer">
                {item.name}
              </div>
            ))}
          </div>
